...
namespace Calculations {
       function calculateCouplingConstant(energy: number): number {
        const alpha = 1 / 137.036; // Fine-structure constant
        return alpha / (1 - (alpha / (3 * Math.PI)) * Math.log(energy / 0.511));
      }
  
       function calculateCrossSectionElectronPositron(energy: number): number {
...
namespace Simulations {
       function simulateElectronPositronAnnihilation(): Interaction {
        return {
...
function simulateComptonScattering(): Interaction {
        return {
          type: InteractionType.Scattering,
